...
summary(as.factor(tb_contact_demo$Discharge_Type__c))
...
tb_contact_demo$Length_of_Service_bin <- ifelse(tb_contact_demo$Length_of_Service<=length_1,1,
...
# We can reduce dimensionality by eliminating outlier categories
summary(as.factor(tb_contact_demo$last_service_era))
tb_contact_demo_c <- filter(tb_contact_demo_c,last_service_era  == "currently_serving"|
                              last_service_era  == "gulf_war_era"|
                              last_service_era  == "post_vietnam_peacetime"|
                              is.na(last_service_era))
summary(as.factor(tb_contact_demo_c$last_service_era))

summary(tb_contact_demo$Length_of_Service_bin)
tb_contact_demo_c <- filter(tb_contact_demo_c, Length_of_Service_bin  == "1"|
                              Length_of_Service_bin  == "2"|
                              Length_of_Service_bin  == "3"|
                              Length_of_Service_bin  == "4"|
                              Length_of_Service_bin  == "5"|
                              Length_of_Service_bin  == "6"|
                              Length_of_Service_bin  == "7"|
                              Length_of_Service_bin  == "8"|
                              Length_of_Service_bin  == "9"|
                              Length_of_Service_bin  == "10"|
                              Length_of_Service_bin  == "11"|
                              is.na(Length_of_Service_bin))


#There are 923 free form answers in Highest level of education.  We can ommit these and keep the standardized responses
levels(as.factor(tb_contact_demo$Highest_Level_of_Education_Completed__c))
summary(as.factor(tb_contact_demo$Highest_Level_of_Education_Completed__c))

tb_contact_demo_c <- filter(tb_contact_demo_c,Highest_Level_of_Education_Completed__c  == "High School/GED"|
                          Highest_Level_of_Education_Completed__c  == "Associate degree (A.A., A.S., etc.)"|
                          Highest_Level_of_Education_Completed__c  == "Some College"|
                          Highest_Level_of_Education_Completed__c  == "College/University"|
                          Highest_Level_of_Education_Completed__c  == "Masters"|
                          Highest_Level_of_Education_Completed__c  == "Degree in Progress - Bachelors"|
                          Highest_Level_of_Education_Completed__c  == "Technical Training"|
                          Highest_Level_of_Education_Completed__c  == "4 year college"|
                          Highest_Level_of_Education_Completed__c  == "2 year college"|
                          Highest_Level_of_Education_Completed__c  == "HIgh School"|
                          ...
                          is.na(Highest_Level_of_Education_Completed__c))

#Clearly some of the levels in the education column are redundant. We should force duplicates into one unique column. 
levels(as.factor(tb_contact_demo_c$Highest_Level_of_Education_Completed__c))
      
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "HIgh School"] <- "High School/GED"
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "2 year college"] <- "2 Year Degree (AA, AS, etc.)"
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "Associate degree (A.A., A.S., etc.)"] <- "2 Year Degree (AA, AS, etc.)"
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "4 year college"] <- "4 Year Degree (BA, BS, etc.)"
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "Degree in Progress - Bachelors"] <- "Some College"
tb_contact_demo_c$Highest_Level_of_Education_Completed__c[tb_contact_demo_c$Highest_Level_of_Education_Completed__c == "Masters"] <- "Post-Graduate Degree (MA, MS, JD, etc.)"
levels(as.factor(tb_contact_demo_c$Highest_Level_of_Education_Completed__c))
summary(as.factor(tb_contact_demo_c$Highest_Level_of_Education_Completed__c))

# tb_contact_demo_c$Status__c[tb_contact_demo_c$Status__c == "Full time"] <- "Full time (35 hours a week or more)"
# tb_contact_demo_c$Status__c[tb_contact_demo_c$Status__c == "Under Employed (Insufficient Income)"] <- "Under employed - Insufficient income"
# tb_contact_demo_c$Status__c[tb_contact_demo_c$Status__c == "Contract/Temporary"] <- "Temporary/Contract Employee"
# tb_contact_demo_c$Status__c[tb_contact_demo_c$Status__c == "Enrolled in a full time higher education program"] <- "Enrolled in higher education program (full or part time)"
# tb_contact_demo_c$Status__c[tb_contact_demo_c$Status__c == "Enrolled part time in a higher education program"] <- "Enrolled in higher education program (full or part time)"
# summary(as.factor(tb_contact_demo_c$Status__c))
# 
# tb_contact_demo_c <- filter(tb_contact_demo_c,Status__c  == "Active Duty"|
#                               Status__c  == "Employed"|
#                               Status__c  == "Enrolled in higher education program (full or part time)"|
#                               Status__c  == "Full time (35 hours a week or more)"|
#                               #Status__c  == "Full time,Enrolled part time higher education"|
#                               Status__c  == "Part time (less than 35 hours a week)"|
#                               Status__c  == "Pending Medical Separation"|
#                               Status__c  == "Student - Not seeking full time employment"|
#                               Status__c  == "Temporary/Contract Employee"|
#                               Status__c  == "Under employed - Insufficient income"|
#                               Status__c  == "Unemployed"|
#                               is.na(Status__c))
# 
# summary(as.factor(tb_contact_demo_c$Status__c))

#Additional filtering from Kayla's code_______________________________________________________________________________________________________
...
# cleaned_data <- filter(cleaned_data, !is.na(Length_of_Service))
# summary(cleaned_data)
...
#Recoding Experiment______________________________________________________________________________________
# #Recoding Ordinal variables. Dependent Spouses are 0 (no paygrade) and Prefer not to answer will get a median grade
# summary(cleaned_data$Service_Rank__c)
# cleaned_data$Service_Rank__c_recode <- recode_factor(cleaned_data$Service_Rank__c,"Dependent Spouse"="0",
#                         "E-1"="1","E-2"="2","E-3"="3","E-4"="4","E5"="5","E-5"="5","E-6"="6","E-7"="7","E-8"="8",
#                         "E-9"="9","O-1"="10","O-2"="11","O-3"="12","O-4"="13","O-5"="14","Prefer not to answer"="15",
#                         "O-6"="16","O-7"="17","O-8"="18","O-9"="19","O-10"="20","W-1"="21","W-2"="22","W-3"="23",
#                         "W-4"="24","W-5"="25","CW"="26","CW-2"="27","CW-3"="28","CW-4"="29","CW-5"="30",
#                         "GS-5"="31")
# 
# cleaned_data$Highest_Level_of_Education_Completed__c_recode <- recode_factor(cleaned_data$Highest_Level_of_Education_Completed__c,
#                                                                              "High School/GED"="1","2 Year Degree (AA, AS, etc.)"="2",
#                                                                              "4 Year Degree (BA, BS, etc.)"="3","Post-Graduate Degree (MA, MS, JD, etc.)"="4",
#                                                                              "Doctorate (PhD, MD, etc.)"="4")
#                                                                              

#summary(cleaned_data$Highest_Level_of_Education_Completed__c_recode)

#drop columns that have been recoded
#cleaned_data$Service_Rank__c <- NULL
#cleaned_data$Highest_Level_of_Education_Completed__c <- NULL
...
# tempData <- mice(cleaned_data[,-12],m=1,maxit=10,meth='cart',seed=42)
# summary(tempData)
# 
...
# final_data <- cbind(completedData, cleaned_data[12])
...
#Service_Branch__c == "Army"
#Service_Rank__c == "E-4"
#Purple_Heart_Recipient__c == "No"
#Highest_Level_of_Education_Completed__c == "High School/GED"
#Status__c == "Unemployed"
#last_service_era == "gulf_war_era"
#Length_of_Service_bin == 2

# cleaned_data$Service_Branch__c[is.na(cleaned_data$Service_Branch__c)] <- as.factor("Army")
# cleaned_data$Service_Rank__c[is.na(cleaned_data$Service_Rank__c)] <- as.factor("E-4")
# cleaned_data$Purple_Heart_Recipient__c[is.na(cleaned_data$Purple_Heart_Recipient__c)] <- as.factor("No")
# cleaned_data$Highest_Level_of_Education_Completed__c[is.na(cleaned_data$Highest_Level_of_Education_Completed__c)] <- as.factor("High School/GED")
# cleaned_data$Status__c[is.na(cleaned_data$Status__c)] <- as.factor("Unemployed")
# cleaned_data$last_service_era[is.na(cleaned_data$last_service_era)] <- as.factor("gulf_war_era")
# cleaned_data$Length_of_Service_bin[is.na(cleaned_data$Length_of_Service_bin)] <- as.factor("2")

cleaned_data$Race__c[is.na(cleaned_data$Race__c)] <- "Blank"
cleaned_data$Gender__c[is.na(cleaned_data$Gender__c)] <- "Blank"
cleaned_data$Service_Branch__c[is.na(cleaned_data$Service_Branch__c)] <- "Blank"
cleaned_data$Service_Rank__c[is.na(cleaned_data$Service_Rank__c)] <- "Blank"
#cleaned_data$Purple_Heart_Recipient__c[is.na(cleaned_data$Purple_Heart_Recipient__c)] <- "Blank"
cleaned_data$Highest_Level_of_Education_Completed__c[is.na(cleaned_data$Highest_Level_of_Education_Completed__c)] <- "Blank"
#cleaned_data$Status__c[is.na(cleaned_data$Status__c)] <- "Blank"
cleaned_data$last_service_era[is.na(cleaned_data$last_service_era)] <- "Blank"
cleaned_data$Length_of_Service_bin[is.na(cleaned_data$Length_of_Service_bin)] <- "Blank"
...
#cleaned_data$Length_of_Service <- as.numeric(cleaned_data$Length_of_Service)
...
fwrite(final_data, file = "complete_data_blank.csv")
